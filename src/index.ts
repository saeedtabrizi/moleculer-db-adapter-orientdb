...
export class OrientDBAdapter<E> implements DbAdapter {
    ...
    public async connect(): Promise<void> {
        ...
        try {
            ...
            try {
                ...
                this.service.logger.error(`The Class '${dataClass.name}' not be created`);
            }
            if (dataClass.properties && cls) {
                ...
                for (const key in dataClass.properties) {
                    try {
                        ...
                        this.service.logger.info(`The Property '${cls.name}.${key}' has been created`);
                    } catch (error) {
                        this.service.logger.error(`The Property '${cls.name}.${key}' not be created`);
                    }
                }
            }
            if (dataClass.indexes) {
                for (const item of dataClass.indexes) {
                    try {
                        ...
                        this.service.logger.error(`The Index '${item.name}' not be created`);
                    }
                }
            }
            if (dataClass.sequences) {
                ...
                for (const item of ss) {
                    ...
                    if (!s) {
                        ...
                        this.service.logger.info(`The Sequence '${c.name}' has been created`);
                    }
                    if (s) {
                        this.sequences[k] = s;
                        this.service.logger.info(`The Sequence '${c.name}' has been attached`);
                    }
                }
            }
     ...
     * @memberof OrientDBAdapter
     */
    public async updateMany<T, Q extends QueryOptions<T> = any>(query: Q, update: T): Promise<number> {
        const svc = this.service;
        const db: orientjs.ODatabase = svc && svc.adapter.database;
        try {
            // const s = await this.client.sessions();
     ...
     * @returns {OQuery<T>}
     */
    public createCursor<T = any, C extends CursorOptions<T> = any, Q = orientjs.OQuery<T>>(
        params?: C,
        isCounting = false,
    ): Q {
        const svc = this.service;
                    ...
                    if (params.searchFields) {
                        sfields =
                            typeof params.searchFields === 'string'
                                ...
                                : params.searchFields;
                        const f = Object.fromEntries(sfields.map(xx => [xx, params.search]));
                        q = q.where(f);
                    }
                } else {
                    if (params.query) {
